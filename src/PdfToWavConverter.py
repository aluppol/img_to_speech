...
class PdfToWavConverter:
    def __init__(self):
        ...
        self.__text_assembler = TextAssembler()
    ...
    def convert_pdf_to_wav(self, pdf_path: str, wav_output_dir_path: str):
        featured_words_pages = [featured_words_page for featured_words_page in self.__text_extractor.extract(pdf_path)]
        featured_book = FeaturedBook([self.__text_preprocessor.preprocess_page(featured_words_page) for featured_words_page in featured_words_pages])
        feature_normalizer = self.__books_feature_normalizers_manager.load_or_init_normalizer('roadto', featured_book)   # get_file_name_from_path(pdf_path)
